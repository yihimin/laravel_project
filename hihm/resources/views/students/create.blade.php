...
<br>
	...
	<form name="form1" method="post" action="{{ route('students.store') }}{{ $tmp }}">
     @csrf
	<table class="table table-sm table-bordered mymargin5">
		...
		<tr>
			...
			<td width="80%" align="left">
				<div class="d-inline-flex">
					<input type="text" name="phone" size="15" maxlength="11" value="{{ old('phone') }}" class="form-control form-control-sm">
				</div>
			...
			<td width="80%" align="left">
				<div class="d-inline-flex">
					<select name="ban" class="form-control form-control-sm">
						<option value="국화반" {{ old('ban') == '국화반' ? 'selected' : '' }}>국화반</option>
						<option value="장미반" {{ old('ban') == '장미반' ? 'selected' : '' }}>장미반</option>
						<option value="백합반" {{ old('ban') == '백합반' ? 'selected' : '' }}>백합반</option>
						<option value="튤립반" {{ old('ban') == '튤립반' ? 'selected' : '' }}>튤립반</option>
					</select>
				</div>
