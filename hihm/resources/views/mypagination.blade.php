@if (isset($paginator) && $paginator->lastPage() > 1)
	...
	@if ($paginator->currentPage() > 1)		<!-- 처음, 이전 -->
		<li class="page-item">
			<a class="page-link" href="{{ $paginator->url(1) }}" aria-label="First">
				<span aria-hidden="true" style="color:#8f9779">◀</span> 
			</a>
		</li>
	...
	@if( !$isCurrentPage)
			<a class="page-link" href="{{ $paginator->url($i) }}" style="color:#556b2f">{{ $i }}</a> 
	@else
			<a class="page-link" href="#" style="color:white;background-color:#a3b18a">{{ $i }}</a> 
	@endif
